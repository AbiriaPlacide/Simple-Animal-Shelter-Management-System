...
Mainwin::Mainwin() : shelter{new Shelter{"Mavs Animal Shelter"}} {
	...
	// File NEW MENU
	Gtk::MenuItem* file_new = Gtk::manage(new Gtk::MenuItem("_New", true));
	file_new->signal_activate().connect([this] {this->on_file_new_click();});
	...
	//////////////////////////////////////
	//HELP MENU//
	Gtk::MenuItem *help = Gtk::manage(new Gtk::MenuItem("_Help", true));
	menubar->append(*help);
	Gtk::Menu *aboutmenu = Gtk::manage(new Gtk::Menu());
	help->set_submenu(*aboutmenu);
...
{
    status("new shelter created!");
	delete shelter;
	shelter = new Shelter{"Mavs Anima Shop"};
}

void Mainwin::on_help_click()
{
	std::cout << "Clicked!" << std::endl;
	Gtk::AboutDialog dialog = Gtk::AboutDialog(); 
	dialog.set_transient_for(*this);
	dialog.set_program_name("MAVS PET SHOT");
	std::vector < Glib::ustring> author = {"*Mostly By George F. Rice\nAbiria Placide"};
	std::vector < Glib::ustring> comments = {"A MAVS shop were one can adopt animals of different types.\nThe Menu Bar is self explenatory"};
	dialog.set_authors(author);
	dialog.set_comments("A MAVS shop were one can adopt animals of different types.\nThe Menu Bar is self explenator");
...
void Mainwin::on_open_click() {
	...
	filter->set_name("MASS Files");
	filter->add_pattern("*.mass");
